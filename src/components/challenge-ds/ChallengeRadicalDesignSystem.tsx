...
export function ChallengeRadicalDesignSystem({ locale }: { locale: string }) {
  ...
  return (
    <div className="bg-[#ede7da] text-[#0f172a]">
      <div className="relative overflow-hidden border-b border-[#1a2746] bg-[#091327] text-white">
        <div className="absolute inset-0 bg-[linear-gradient(115deg,rgba(49,82,224,0.28)_0%,transparent_34%,transparent_66%,rgba(49,82,224,0.20)_100%)]" />
        ...
        <Container className="relative py-16 md:py-24">
          <div className="grid gap-10 md:grid-cols-[1.18fr_0.82fr] md:items-end">
            <div className="max-w-[820px]">
              <span className="inline-flex items-center gap-3 border border-white/14 bg-white/8 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.32em] text-[#a8b7ff]">
                Challenge Radical DS
                <span className="h-1.5 w-8 bg-[#3152e0]" />
              </span>
              <h1
                className={cn(
                  "mt-7 max-w-[10ch] text-[3.6rem] leading-[0.88] tracking-[-0.06em] text-white md:text-[6.4rem]",
                  fraunces.className,
                )}
              >
                Même système. Beaucoup plus de présence.
              </h1>
              <div className="mt-7 grid gap-6 md:grid-cols-[1.1fr_0.9fr]">
                <p className="max-w-[34ch] text-[1.08rem] leading-8 text-white/74">
                  Cette version pousse plus loin l’identité : plus de contraste, plus
                  d’asymétrie, plus de tension graphique, tout en gardant exactement les
                  mêmes familles de sections.
                </p>
                <div className="border-l border-white/12 pl-5 text-sm leading-7 text-white/60">
                  On quitte franchement l’univers “SaaS propre” pour aller vers une
                  plateforme de gouvernance plus signée, plus mémorable, plus assumée.
                </div>
              </div>
              <div className="mt-10 flex flex-wrap gap-4">
                <ChallengeButton href="#HeroAnimated">Entrer dans la direction</ChallengeButton>
            ...
            <FadeIn>
              <div className="grid gap-4 border border-white/12 bg-white/6 p-5 backdrop-blur-sm md:translate-y-6">
                <ValueStrip label="Rupture" value="Forte, mais système" tone="blue" />
                <ValueStrip label="Couleur" value="Bleu conservé, beaucoup plus affirmé" tone="light" />
                <ValueStrip label="Posture" value="Plateforme stratégique, non template" tone="dark" />
              </div>
            </FadeIn>
          </div>
        </Container>
      ...
      <div className="border-b border-[#1f2d50] bg-[#101b34] text-white">
        <Container className="py-7">
          <div className="grid gap-4 md:grid-cols-[auto_1fr] md:items-start">
            <p className="text-[11px] font-semibold uppercase tracking-[0.32em] text-[#8fa2ff]">
              Same section families
            </p>
            <div className="flex flex-wrap gap-2.5">
              {SECTIONS.map((section) => (
                <a
                  key={section}
                  href={`#${section}`}
                  className="border border-white/14 bg-white/5 px-3 py-1.5 text-sm text-white/72 transition-colors hover:border-[#8fa2ff] hover:bg-white/10 hover:text-white"
                >
                  {section}
                </a>
              ))}
...
function SectionMarker({ name }: { name: string }) {
  const sectionNumber = String(SECTIONS.indexOf(name) + 1).padStart(2, "0");

  return (
    <div id={name} className="border-y border-[#cfc7b9] bg-[#f4eee2]">
      <Container className="py-3">
        <div className="grid grid-cols-[auto_1fr] items-center gap-4">
          <span
            className={cn(
              "text-[0.88rem] font-semibold tracking-[0.26em] text-[#3152e0]",
              fraunces.className,
            )}
          >
            {sectionNumber}
          </span>
          <code className="text-[11px] font-semibold uppercase tracking-[0.34em] text-[#1b2748]">
            {name}
          </code>
        </div>
      </Container>
...
function ChallengeButton({
  ...
  const variants: Record<ButtonVariant, string> = {
    primary:
      "border border-[#3152e0] bg-[#3152e0] text-white shadow-[6px_6px_0_rgba(12,20,39,0.18)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none",
    secondary:
      "border border-[#c8d2ff] bg-white text-[#15213f] shadow-[6px_6px_0_rgba(49,82,224,0.10)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none",
    ghost:
      "border border-white/20 bg-white/8 text-white hover:border-white/40 hover:bg-white/14",
    quiet:
      "border border-[#1b2748] bg-[#0f1930] text-white shadow-[6px_6px_0_rgba(49,82,224,0.18)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none",
  };
  ...
  return (
    <Link
      href={href}
      className={cn(
        "inline-flex items-center justify-center px-6 py-3 text-sm font-semibold uppercase tracking-[0.16em] transition-all duration-200",
        variants[variant],
        className,
      )}
    >
...
function ValueStrip({
  ...
  const tones = {
    blue: "border-[#7691ff] bg-[#3152e0] text-white",
    light: "border-[#d6ddff] bg-[#eef2ff] text-[#15213f]",
    dark: "border-[#3152e0] bg-[#0f1930] text-white",
  };

  return (
    <div className={cn("border px-4 py-4", tones[tone])}>
...
function ButtonShowcase({ demoHref }: { demoHref: string }) {
  return (
    <section className="py-16 md:py-20">
      <Container>
        <FadeIn>
          <div className="grid gap-0 overflow-hidden border border-[#1a2746] bg-[#091327] text-white md:grid-cols-[0.84fr_1.16fr]">
            <div className="border-b border-white/10 px-8 py-10 md:border-b-0 md:border-r md:px-10 md:py-12">
              <Eyebrow className="border-white/15 bg-white/8 text-[#9fb0ff]">Commandes</Eyebrow>
              <DisplayTitle className="mt-5 max-w-[9ch] text-white">
                Un bouton qui agit comme une signature.
              </DisplayTitle>
              <p className="mt-5 max-w-[38ch] text-[1rem] leading-8 text-white/72">
                Plus angulaire, plus tendu, plus assumé. On sort du CTA “gentil” pour
                quelque chose de plus mémorable et plus éditorial.
              </p>
            </div>

            <div className="grid gap-5 bg-[#f4eee2] p-6 text-[#0f172a] md:p-8">
              ...
              <div className="grid gap-4 border border-[#1a2746] bg-[#3152e0] p-6 text-white md:grid-cols-[1fr_auto] md:items-end">
                <p className="max-w-[34ch] text-sm leading-7 text-white/78">
                  Même API de composant, mais un langage beaucoup plus tranché. Le bouton
                  devient un vrai marqueur de caractère.
                </p>
...
function HeroAnimatedShowcase({
  ...
  return (
    <section className="py-16 md:py-24">
      <Container>
        <FadeIn>
          <div className="overflow-hidden border border-[#1a2746] bg-[linear-gradient(135deg,#081224_0%,#10204a_40%,#3152e0_100%)] text-white shadow-[0_42px_100px_rgba(17,25,47,0.18)]">
            <div className="grid gap-10 px-8 py-10 md:grid-cols-[0.82fr_1.18fr] md:px-10 md:py-12">
              <div className="flex flex-col justify-between md:pt-6">
                <div>
                  <Eyebrow className="border-white/15 bg-white/8 text-[#9fb0ff]">
                    Hero éditorial
                  </Eyebrow>
                  <h2
                    className={cn(
                      "mt-6 max-w-[8ch] text-[3.6rem] leading-[0.88] tracking-[-0.06em] md:text-[5.8rem]",
                      fraunces.className,
                    )}
                  >
                    Le PPM qui ose enfin prendre de la place.
                  </h2>
                  <p className="mt-6 max-w-[34ch] text-[1.02rem] leading-8 text-white/78">
                    Ici, la hero ne rassure pas seulement. Elle affirme une marque. Plus de
                    contraste, plus de silence, plus de décision dans la composition.
                  </p>
                  <div className="mt-8 flex flex-wrap gap-4">
                    <ChallengeButton href={demoHref}>Réserver une démo</ChallengeButton>
                ...
                <div className="mt-12 grid gap-4 md:grid-cols-3">
                  {[
                    ...
                    <div
                      key={label}
                      className="border border-white/14 bg-black/15 p-4 backdrop-blur-sm"
                    >
              ...
              <div className="space-y-5 md:translate-y-10">
                <div className="flex flex-wrap gap-2 md:justify-end">
                  {heroAnimatedTabs.map((tab, index) => (
                    <button
                      ...
                      className={cn(
                        "border px-3 py-1.5 text-sm uppercase tracking-[0.14em] transition-colors",
                        activeIndex === index
                          ? "border-white bg-white text-[#15213f]"
                          : "border-white/15 bg-black/15 text-white/75 hover:bg-white/10",
                      )}
                    >
                      {tab.label}
                    </button>
                ...
                <div className="overflow-hidden border border-white/14 bg-[#f7f8fc] p-4 text-[#11192f] shadow-[14px_14px_0_rgba(9,19,39,0.26)]">
                  <div className="mb-4 flex items-center justify-between border border-[#e5e8f4] bg-white px-4 py-3">
                    <p className="text-sm font-semibold text-[#15213f]">{activeTab.label}</p>
                    <span className="bg-[#eef2ff] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-[#3152e0]">
                      Active view
                    </span>
                  </div>

                  <div className="relative overflow-hidden border border-[#e3e7f2] bg-[#f2f5fb] shadow-[0_24px_55px_rgba(17,25,47,0.08)]">
                    <div className="relative aspect-[16/10] w-full">
                      <Image
                        src={activeTab.image}
                        alt={`Vue ${activeTab.label}`}
                        fill
...
function FeaturePanel({
  ...
  > = {
    ivory: {
      shell: "border-[#1b2748] bg-white text-[#10182f] shadow-[12px_12px_0_rgba(49,82,224,0.08)]",
      body: "text-[#4b5770]",
      accent: "border border-[#d7ddf4] bg-[#eef2ff] text-[#3152e0]",
      button: "secondary",
      panel: "border-[#1b2748] bg-[#f7f4ee]",
      innerBorder: "border-[#e6e0d6]",
      innerSurface: "bg-white/70",
    },
    blue: {
      shell: "border-[#3152e0] bg-[#3152e0] text-white shadow-[12px_12px_0_rgba(12,20,39,0.16)]",
      body: "text-white/82",
      accent: "border border-white/20 bg-[#0f1930] text-white",
      button: "primary",
      panel: "border-white/14 bg-white/10",
      innerBorder: "border-white/14",
      innerSurface: "bg-black/10",
    },
    ink: {
      shell: "border-[#3152e0] bg-[#091327] text-white shadow-[12px_12px_0_rgba(49,82,224,0.16)]",
      body: "text-white/74",
      accent: "border border-[#3152e0] bg-[#3152e0] text-white",
      button: "ghost",
      panel: "border-white/14 bg-white/8",
      innerBorder: "border-white/14",
      innerSurface: "bg-black/15",
    },
  };
    ...
    <section className="py-6 md:py-8">
      <Container>
        <FadeIn>
          <div
            className={cn(
              "grid gap-10 border p-8 md:grid-cols-[0.9fr_1.1fr] md:p-10",
              styles.shell,
            )}
          >
            <div className={cn("flex flex-col justify-center", reversed && "md:order-2")}>
              <span
                className={cn(
                  "inline-flex w-fit items-center px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.3em]",
                  styles.accent,
                )}
              >
                {badge}
            ...
            <div className={cn(reversed && "md:order-1")}>
              <div className={cn("border p-4", styles.panel)}>
                <div
                  className={cn(
                    "mb-4 flex items-center justify-between border px-4 py-3 text-sm",
                    styles.innerBorder,
                    styles.innerSurface,
                  )}
                >
                ...
                <div className={cn("relative overflow-hidden border", styles.innerBorder)}>
                  <div className="relative aspect-[16/10] w-full bg-[#eef1f7]">
                    <Image
                      src={image}
                      alt={imageAlt}
                      fill
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {quoteCards.map((item, index) => (
              <div
                key={item.quote}
                className={cn(
                  "rounded-[30px] border p-8 shadow-[0_18px_50px_rgba(17,25,47,0.06)]",
                  index === 0
              ...
              >
                <p
                  className={cn(
                    "text-[3rem] leading-none tracking-[-0.06em]",
                    fraunces.className,
                ...
                <p className="mt-3 max-w-[24ch] text-[1.35rem] leading-9">
                  {item.quote}
                </p>
      ...
      <Container>
        <FadeIn>
          ...
          >
            ...
            <div className={cn(reversed && "md:order-1")}>
              <div className="rounded-[28px] border border-[#e6e0d6] bg-[#faf8f3] p-4">
                <div className="relative overflow-hidden rounded-[22px] border border-[#e6e0d6]">
                  <div className="relative aspect-[16/10] w-full bg-[#eef2fb]">
                    <Image
                      src={image}
                      alt={imageAlt}
                      fill
      ...
      <Container>
        <FadeIn>
          <div className="grid gap-10 overflow-hidden rounded-[38px] border border-[#d6dcf5] bg-white shadow-[0_24px_60px_rgba(17,25,47,0.08)] md:grid-cols-[1.05fr_0.95fr]">
            <div className="relative min-h-[420px] border-b border-[#e5e8f2] bg-[#eff3ff] md:border-b-0 md:border-r">
              <Image
                src="/assets/images/Presentation%20cadrage%20screen.webp"
                alt="Écran cadrage projet AirSaaS"
                fill
...
function HeroTabbedShowcase({
  ...
  return (
    <section className="py-16 md:py-20">
      <Container>
        <FadeIn>
          <div className="overflow-hidden border border-[#3152e0] bg-[#081224] text-white shadow-[14px_14px_0_rgba(49,82,224,0.18)]">
            <div className="grid gap-10 px-8 py-10 md:grid-cols-[0.72fr_1.28fr] md:px-10 md:py-12">
              <div>
                <Eyebrow className="border-white/15 bg-white/8 text-[#9fb0ff]">Hero tabbed</Eyebrow>
                <DisplayTitle className="mt-5 max-w-[8ch] text-white">
                  Une hero plus brutale, plus mémoire de marque.
                </DisplayTitle>
                <p className="mt-5 max-w-[36ch] text-[1rem] leading-8 text-white/70">
                  Le module reste clair et réutilisable, mais le dispositif visuel devient
                  plus fort : plus de blocs, plus de masses, plus de contraste.
                </p>
              ...
              <div className="border border-white/10 bg-white/8 p-4 md:translate-x-6 md:translate-y-6">
                <div className="mb-4 flex items-center justify-between border border-white/10 bg-black/20 px-4 py-3">
                  <div>
                ...
                <div className="relative overflow-hidden border border-white/10">
                  <div className="relative aspect-[16/10] w-full bg-[#18264b]">
                    <Image
                      src={activeTab.image}
                      alt={`Vue ${activeTab.label}`}
                      fill
          ...
          <div className="grid gap-8 rounded-[34px] border border-[#ddd7cb] bg-white p-8 md:grid-cols-[0.88fr_1.12fr] md:p-10">
            ...
            <div className="rounded-[28px] border border-[#e6e0d6] bg-[#faf8f3] p-4">
              <div className="relative overflow-hidden rounded-[22px] border border-[#e6e0d6]">
                <div className="relative aspect-[16/10] w-full bg-[#eef2fb]">
                  <Image
                    src="/assets/images/Automation%20-%20integrations.webp"
                    alt="Intégrations et automatisations AirSaaS"
                    fill
                    className="object-cover object-top"
                  />
                </div>
...
function FinalCtaShowcase({ demoHref }: { demoHref: string }) {
  return (
    <section className="py-16 md:py-20">
      <Container>
        <FadeIn>
          <div className="overflow-hidden border border-[#3152e0] bg-[linear-gradient(135deg,#3152e0_0%,#11204a_100%)] px-8 py-12 text-white shadow-[16px_16px_0_rgba(12,20,39,0.18)] md:px-12 md:py-14">
            <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-end">
              <div>
                <Eyebrow className="border-white/15 bg-white/8 text-[#b5c2ff]">Final CTA</Eyebrow>
                <DisplayTitle className="mt-5 max-w-[9ch] text-white">
                  Prêt pour une version vraiment moins sage ?
                </DisplayTitle>
                <p className="mt-5 max-w-[54ch] text-[1.02rem] leading-8 text-white/80">
                  Cette V2 radicale pousse plus loin la signature. Toujours système,
                  toujours landing, mais avec une identité qui ne s’excuse plus d’exister.
                </p>
              </div>
              <div className="flex flex-wrap gap-4">
                <ChallengeButton href={demoHref} variant="secondary">
...
function Eyebrow({
  ...
  return (
    <span
      className={cn(
        "inline-flex w-fit items-center border border-[#d7ddf4] bg-[#eef2ff] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.32em] text-[#3152e0]",
        className,
      )}
    >
      {children}
...
function DisplayTitle({
  ...
  return (
    <h2
      className={cn(
        "text-[2.7rem] leading-[0.9] tracking-[-0.06em] md:text-[4.8rem]",
        fraunces.className,
        className,
      )}
    >
