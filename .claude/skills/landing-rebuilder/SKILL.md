...
16. **3-column colored-bar bullets under a heading → `feature-stacked` (`<FeatureSectionStacked>`), NOT `section-heading` + separate `feature-split`.** When the live shows a section with H2/H3 + subtitle + **3-4 short bullets in a row with vertical colored bars** + optional illustration below, the canonical DS pattern is **`<FeatureSectionStacked>`** in ONE component — NOT a `section-heading` followed by a `feature-split`. **Detection signals in raw HTML**: (a) `<blockquote class="block-quote--columns">` (3-col blockquotes — primary signal), (b) `<li class="list-item">` inside `<ul class="list align-horizontal--center">` (alternative list form), (c) `<em class="heading__pill">` decorative first-letter circle on the outer H2 (Webflow ornament — DROP per "DS is source of truth"). The `listItems` prop of `<FeatureSectionStacked>` accepts 3-6 strings. `imageSrc` is OPTIONAL (Storybook has `Without Image` story). When the live nests an outer H2 with `heading__pill` + an inner H3 + bullets, MERGE them: outer H2 is decorative and dropped; inner H3 becomes `titleGradient`. Verified incident 2026-05-14: mapped `/fr/produit/capacity-planning` "Les Scénarios" + "Trouvez le scénario qui fonctionne" as `section-heading` + `feature-split` (2 rows) but the live has `<blockquote class="block-quote--columns">` × 3 + image → user flagged via Storybook screenshot. Correct mapping = ONE `<FeatureSectionStacked>`. Same pattern affects: capacity-planning (3 sections), reporting-projet (1), traduction-one-click-avec-deepl (1), budget (2). **Mitigation**: ALWAYS grep the raw HTML for `block-quote--columns` and `class="list-item"` BEFORE mapping any feature section.
