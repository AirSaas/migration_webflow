...
17. **`imageSize="narrow"` vs default — detect by Webflow class `small_pic` + image extension.** When mapping a `<FeatureFrame>` (especially `richContent` editorial sections), pick `imageSize` from TWO concrete signals in the raw HTML, NOT from gut feel: (a) **Webflow class signal** — `<div class="container__features__section small_pic">` → `imageSize="narrow"` (editorial graphic, ~33% width). Without `small_pic` → `imageSize="default"` (product screenshot, ~60% width). (b) **File extension signal** — `.svg` files are editorial schemas/illustrations → `narrow`. `.webp` files are product screenshots → `default`. Both signals usually agree; when they disagree, trust the Webflow class. Verified incident 2026-05-14: mapped 5 sections of `/fr/solution/management-de-portefeuille-projet` "Les 5 règles d'or" all as `narrow` but ONLY 3 are (sections #20 Réduire, #23 Ritualiser, #24 Renforcer — all with `small_pic` + `.svg`). The other 2 (#21 Définir un ordre, #22 Privilégier l'achèvement) use `.webp` screenshots and lack `small_pic` → `imageSize="default"`. **Mitigation**: grep raw HTML for `class="container__features__section small_pic"` to identify narrow sections before mapping, AND verify the image filename extension (.svg vs .webp).
