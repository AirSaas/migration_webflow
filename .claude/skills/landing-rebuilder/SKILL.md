...
7. **Count visible elements in BOTH text AND image passes.** Extract text via the visible-text pass, but ALSO extract image counts in the same range via `<img src>` regex. If they disagree (e.g. 5 customer logos visible in `<img>` but 0 in visible text because they have no alt), the visual layer takes precedence — render as logo-bar / image grid, not as text-only block. Verified incident 2026-05-14: mapped `/fr/lp/ppm` "Ils nous font confiance" as `stats` (3 KPIs) and missed the 5 customer logos rendered as bare `<img>` tags between heading and KPIs; ALSO missed a 4th KPI because the text-pass window was too narrow.
